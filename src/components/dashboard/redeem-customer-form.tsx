...
export type RedeemMode = "free-drink-voucher" | "voucher" | "custom";

interface RedeemCustomerFormProps {
  customerId: string;
  onCustomerIdChange?: (customerId: string) => void;
  showCustomerSelect?: boolean;
  defaultMode?: RedeemMode;
  onSuccess?: () => void;
  onCancel?: () => void;
  submitClassName?: string;
}

export function RedeemCustomerForm({
  customerId,
  onCustomerIdChange,
  showCustomerSelect = true,
  defaultMode = "free-drink-voucher",
  onSuccess,
  onCancel,
  submitClassName,
}: RedeemCustomerFormProps) {
  const { customers, redeemPoints, redeemVoucher, redeemFreeDrinkVoucher } =
    useLoyalty();
  const [mode, setMode] = useState<RedeemMode>(defaultMode);
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      {showCustomerSelect && onCustomerIdChange && (
        <CustomerSelectField
      ...
      {selected && (
        <div className="rounded-lg border bg-muted/40 px-3 py-2 text-sm">
          ...
          <p className="text-muted-foreground">
            ...
            {selected.freeDrinkVouchersAvailable} × {freeDrinkVoucherReward.label}
      ...
      <div className="grid gap-2">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          >
            ...
            <span className="text-sm text-muted-foreground">From stack</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span className="text-sm text-muted-foreground">Any points</span>
          </button>
        </div>
      </div>
      ...
      {selected &&
        ...
        maxFreeDrink > 0 && (
          <p className="text-sm text-muted-foreground">
            Stack after:{" "}
            {Math.max(0, selected.freeDrinkVouchersAvailable - effectiveQty)}
          </p>
        )}

      {selected && mode === "custom" && pointsNum > 0 && (
        <p className="text-sm text-muted-foreground">
          Balance after: {Math.max(0, selected.points - pointsNum)} pts · consecutive
          streak resets
        </p>
      )}
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        >
          {isSubmitting ? "Redeeming..." : submitLabel}
        </Button>
      </div>
    </form>
  );
